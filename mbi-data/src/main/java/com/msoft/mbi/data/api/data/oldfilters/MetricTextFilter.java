...
public class MetricTextFilter extends MetricFilter {
    ...
    public String getFormattedValue() throws BIException {
        int x = this.getCondition().getField().getNumDecimalPositions();

        NumberFormat nf = NumberFormat.getInstance(Locale.GERMANY);
        nf.setMaximumFractionDigits(x);
        nf.setMinimumFractionDigits(x);

        String valorSQL = (String) this.applyValues(this.getCondition().getSQLValue(), 0);
